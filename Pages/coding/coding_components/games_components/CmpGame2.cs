...
namespace BlazoredPortfolio.Pages.coding.coding_components.games_components {
    public partial class CmpGame2 {
        ...
        public void GameContact() {
            NavManager.NavigateTo("/contact/game2");
        }

        public void On_SlideShow_SRC(int tiSlideSrc) {
            //SINCE THIS GAME ONLY HAS 8 IMAGES FOR ITS SLIDESHOW, CHECK AND REUSE THE INT VAR
            _slideShow_Index += tiSlideSrc;
            if (_slideShow_Index < 1) { _slideShow_Index = 1; }
            if (_slideShow_Index > 7) { _slideShow_Index = 7; }
        ...
        public string URLCopyButtonText { get; set; } = "Copy Game 2 URL ";
